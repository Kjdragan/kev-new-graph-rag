...
log_filename = f"test_ingestion_no_ontology_{Path(__file__).stem}.log"
logger.add(
    sys.stderr, 
    level="INFO",
...
async def main():
    logger.info("Starting test ingestion: NO custom ontology.")

    try:
        # Load configuration (specifically for Gemini model config)
        # This assumes your config.yaml and .env are set up for gemini_suite.pro_model
        config = get_config()
        ...
        # The GraphExtractor itself needs the LLM config for its internal Graphiti instance
        ...
        logger.info("Extracting graph with NO custom ontology...")
        
        # Call extract with empty lists for ontology_nodes and ontology_edges
        extraction_results = await graph_extractor.extract(
            text_content=sample_text,
            ontology_nodes=[],  # No custom node types
            ontology_edges=[],  # No custom edge types
            group_id="test_no_ontology_group",
            episode_name_prefix="test_no_ontology_ep"
        )
